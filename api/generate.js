...
애매한 경우에는 "업무의 최종 실행 주체"가 어디인지를 기준으로 판단할 것.
...
■ 개발실 (development)
  - 클라이언트/서버 코드 개발 및 구현
  - API 개발, 서비스 연동, 인프라 아키텍처 구축
  - 빌드/배포, CI/CD, DevOps 작업
  - 버그 수정, 성능 최적화, 리팩토링
  - DB 설계/마이그레이션, 서버 설정 변경
  - 예시: "전투 서버 비동기 전환 구현", "API v2 엔드포인트 개발", "인벤토리 버그 핫픽스", "CI/CD 자동화 설정"
...
■ 개발관리실 (management)
  ...
  - 프로세스 개선, 문서화, 위키 정리
  - 외부 업체 계약/조율, 라이선스 관리
  - 예시: "QA 인력 온보딩", "에셋 핸드오프 프로세스 문서화", "외부 사운드 스튜디오 계약 진행", "마일스톤 백로그 정리"

■ AI센터 (ai)
  - LLM, LM 등 AI 모델 관련 연구/개발/테스트
  - TTS, STT, 음성 합성 관련 작업
  - 다국어 모델 적용, 번역 모델 개발/테스트
  - 프롬프트 엔지니어링, AI 품질 테스트
  - AI 기반 콘텐츠 생성 (NPC 대화, 자동 번역 등)
  - 예시: "TTS 모델 한국어 품질 테스트", "NPC 대화 생성 프롬프트 최적화", "다국어 번역 모델 정확도 검증", "LLM 응답 품질 벤치마크"

★ 분류 판단 원칙:
1. "기획서를 쓴다/설계한다" → 기획실
2. "코드를 짠다/구현한다/수정한다/배포한다" → 개발실
3. "일정을 관리한다/QA를 한다/프로세스를 정리한다/외부 조율한다" → 개발관리실
4. "AI 모델을 만든다/테스트한다/프롬프트를 짠다/번역 품질을 본다" → AI센터
5. 하나의 태스크가 여러 부서에 걸칠 경우, "최종 산출물을 만드는 부서"에 배정

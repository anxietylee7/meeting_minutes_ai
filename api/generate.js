...
★★★ 절대 줄글/문단 형태로 작성 금지 ★★★
반드시 아래 예시와 같은 계층형 불릿 리스트 형태로만 작성할 것.

형식 규칙:
1. 대주제는 "- **주제명**" 형태 (볼드 필수)
2. 소주제는 반드시 공백 2칸 + "- " 로 들여쓰기
3. 각 소주제는 한 문장으로, 명사형 종결 (예: ~확정, ~예정, ~진행, ~필요)
4. 소주제 뒤에 줄글 설명을 붙이지 말 것. 한 줄에 한 항목만.
5. 회의에서 논의된 모든 내용을 빠짐없이 포함
6. 중요한 수치, 이름, 날짜는 반드시 포함

올바른 예시 (반드시 이 형태를 따를 것):
- **용병 성향 LM 및 TTS 처리**
  - 용병 TTS는 별도 제작 없이 기존 3개 TTS 사용 확정
  - 용병 성향 8개에 맞춰 LM 테이블만 분리 생성
  - 기획에서 TTS 매핑 방식 확정 후 AI센터에 전달 예정
- **TTS 퀄리티 및 모델 개발**
  - 비실시간 모델 작업 중이나 실시간 모델 개발 필요성 확인
  - 실시간/비실시간 병행 적용 및 퀄리티 기준 산정 예정
  - 용병 TTS 개선 태스크 CBT 전 마무리 목표
- **다국어 TTS 및 딜레이 이슈**
  - 한국어 TTS 캐싱 서버 활용, 텍스트는 외국어로 생성
  - 최대 3초 딜레이 → 용병 발화 타이밍 고려 후 TTS 제외 여부 테스트 예정

잘못된 예시 (이렇게 쓰면 안 됨):
✗ "용병 TTS는 별도 제작 없이 기존 3개 TTS를 사용하고, 용병 성향 8개에 맞춰 LM 테이블만 분리 생성하기로 확정됨 기획에서 TTS 매핑 방식을 확정하여..."
→ 이런 줄글 형태는 절대 금지. 반드시 소주제별로 줄을 나눠서 불릿으로 작성할 것.
